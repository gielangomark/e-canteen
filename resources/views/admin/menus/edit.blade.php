<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.menus.index') }}" class="p-2 rounded-lg text-slate-500 hover:bg-white/5 transition">
            ...
            <form action="{{ route('admin.menus.update', $menu) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        @endif
                        <input id="image" name="image" type="file" accept="image/*" class="w-full text-sm text-slate-400
                            file:mr-4 file:py-2.5 file:px-4
                            file:rounded-xl file:border-0
                            file:text-sm file:font-semibold
                            file:bg-indigo-500/20 file:text-indigo-300
                            hover:file:bg-indigo-500/30 file:transition" />
                        <p class="text-xs text-slate-500 mt-1.5">Kosongkan jika tidak ingin mengubah foto</p>
                ...
                <div class="flex items-center gap-4 mt-8 pt-6" style="border-top: 1px solid rgba(99,102,241,0.1);">
                    <button type="submit" class="px-6 py-3 gradient-primary text-white font-semibold rounded-xl shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all">
                        Perbarui Menu
                    </button>
                    <a href="{{ route('admin.menus.index') }}" class="text-sm font-medium text-slate-500 hover:text-slate-200 transition">Batal</a>
                </div>
            </form>
        </div>
    </div>
