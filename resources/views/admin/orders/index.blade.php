...
<x-slot name="header">
    ...
    <div class="rounded-2xl p-5 mb-6" style="background: rgba(30,27,75,0.5); border: 1px solid rgba(99,102,241,0.12);">
        <form method="GET" action="{{ route('admin.orders.index') }}" class="flex flex-wrap gap-4 items-end">
            ...
            <div>
                ...
                <select name="status" class="rounded-xl shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); color: #e2e8f0;">
    ...
    <div x-data="{ tab: 'istirahat_1' }">
        ...
        <div x-show="tab === 'istirahat_1'" x-transition>
            @include('admin.orders._order-list', ['orderList' => $ordersIstirahat1])
        </div>

        <div x-show="tab === 'istirahat_2'" x-transition>
            @include('admin.orders._order-list', ['orderList' => $ordersIstirahat2])
        </div>
    </div>
</x-app-layout>
