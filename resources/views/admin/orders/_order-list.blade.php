                    ...
                    <div class="flex flex-col gap-2 min-w-[160px]">
                        @if($order->status === 'pending')
                            <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                                ...
                                <button type="submit" class="w-full px-4 py-2.5 gradient-primary text-white text-sm font-semibold rounded-xl shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 transition-all">
                                    🔥 Proses Pesanan
                                </button>
                            </form>
                            <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                                ...
                                <button type="submit" class="w-full px-4 py-2.5 bg-red-500/10 text-red-400 text-sm font-semibold rounded-xl hover:bg-red-500/20 transition-all" style="border: 1px solid rgba(239,68,68,0.2);" onclick="return confirm('Yakin ingin membatalkan pesanan ini?')">
                                    Batalkan
                                </button>
                            </form>
                        @elseif($order->status === 'preparing')
                            <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                                ...
                                <button type="submit" class="w-full px-4 py-2.5 gradient-success text-white text-sm font-semibold rounded-xl shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 transition-all">
                                    ✅ Siap Diambil
                                </button>
                            </form>
                        @elseif($order->status === 'ready')
                            <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST">
                                ...
                                <button type="submit" class="w-full px-4 py-2.5 bg-slate-600 text-white text-sm font-semibold rounded-xl shadow-lg hover:bg-slate-500 transition-all">
                                    🎉 Selesai
                                </button>
                            </form>
                        @endif
