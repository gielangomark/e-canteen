...
<x-slot name="header">Penarikan Dana</x-slot>
    ...
    @if($pendingCount > 0)
        <div class="mb-6 rounded-xl p-4 flex items-center gap-3" style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.2);">
            ...
            <span class="text-sm text-amber-300 font-medium">{{ $pendingCount }} penarikan menunggu persetujuan</span>
        </div>
    @endif
            ...
            <table class="min-w-full">
                <thead>
                    <tr style="border-bottom: 1px solid rgba(99,102,241,0.1);">
                        <th class="px-5 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Kantin</th>
                        <th class="px-5 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Penjual</th>
                        <th class="px-5 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Jumlah</th>
                        <th class="px-5 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Saldo Kantin</th>
                        <th class="px-5 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Catatan</th>
                ...
                <tbody class="divide-y divide-white/5">
                    @forelse($withdrawals as $wd)
                        <tr class="hover:bg-white/5 transition">
                            <td class="px-5 py-4 text-sm font-medium text-white">{{ $wd->canteen->name }}</td>
                            <td class="px-5 py-4 text-sm text-slate-300">{{ $wd->canteen->seller->name ?? '-' }}</td>
                            <td class="px-5 py-4 text-sm font-semibold text-amber-400">Rp {{ number_format($wd->amount, 0, ',', '.') }}</td>
                            <td class="px-5 py-4 text-sm text-emerald-400">Rp {{ number_format($wd->canteen->balance, 0, ',', '.') }}</td>
                            <td class="px-5 py-4 text-sm text-slate-400 max-w-[200px] truncate">{{ $wd->notes ?: '-' }}</td>
                            <td class="px-5 py-4 text-sm text-slate-400">{{ $wd->created_at->format('d M Y H:i') }}</td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4">
                                @if($wd->status === 'pending')
                                    <div class="flex gap-2">
                                        <form action="{{ route('super-admin.withdrawals.approve', $wd) }}" method="POST">
                                            @csrf @method('PATCH')
                                        ...
                                        <form action="{{ route('super-admin.withdrawals.reject', $wd) }}" method="POST">
                                            @csrf @method('PATCH')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center text-slate-500">Belum ada permintaan penarikan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
